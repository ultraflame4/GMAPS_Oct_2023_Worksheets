...
public class Player : MonoBehaviour
{
    ...
    float AngleToPlayer()
    {
        ...
        Vector3 toPlayer = OtherPlayer.transform.position - transform.position;
        DebugExtension.DebugArrow(transform.position, toPlayer, Color.black);
        ...
        float dot = Vector3.Dot(toPlayer.normalized, transform.forward);
        // Skip (toPlayer.magnitude * transform.forward.magnitude) because both vectors are normalised,
        // so would be 1 * 1 = 1 -> which is jst dot / 1 = dot
